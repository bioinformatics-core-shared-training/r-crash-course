...
Like other software (Word, Excel, Photoshop....), R has a default location where it will save files to and import data from. Equivalent to selecting ***File - Save*** or ***File - Open*** from those other tools, we don't have to tell R where we want data and plots etc to be saved. This is known as the *working directory* in R. You can query what R currently considers its working directory by doing:-
...
*Disclaimer 2*:- later in the course, we will see a slightly-nicer way of plotting using `ggplot2`.
